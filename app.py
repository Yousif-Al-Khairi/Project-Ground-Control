...
class AnimationMixin:
    ...
    def black(self, steps=1):
        return self.color(0,0,0, steps=steps)
    def bounce(self):
        return Bounce(self)
    def offset(self, n):
    ...
    def limit(self, steps):
        return Limit(self, steps)
    def fade_in(self, steps):
        return FadeIn(self, steps)
    def random_delay(self, steps):
        return RandomDelay(steps).then(self)
    def then_random_delay(steps):
        return self.then(RandomDelay(steps))
    def last(self):
        if self.is_finite():
            last = None
            for x in self:
...
class Animation(AnimationMixin):
    ...
    def add(self,x):
        ...
        return self
    def is_finite(self):
        return all(type(x)==tuple or x.is_finite() for x in self.a)
    def __len__(self):
        if len(self.a):
            return sum((1 if type(x)==tuple else len(x)) for x in self.a) if self.is_finite() else None
        else:
            return 0
...
class RandomDelay(AnimationMixin):
...
class Interpolate(AnimationMixin):
    def __init__(self, color1=(0,0,0), color2=(0,0,0), steps=50):
        self.color1=color1
        self.color2=color2
        self.steps=steps
    def __len__(self):
...
class FadeIn(AnimationMixin):
    ...
    def __iter__(self):
        ...
        for i in range(self.steps):
            w=i/self.steps
            (r,g,b)=next(it)
            yield (int(w*r),int(w*g),int(w*b))
        for c in it:
            yield c
    def clone(self):
        return FadeIn(self.a, self.steps)
    def last(self):
        return self.a.last()

class Limit(AnimationMixin):
    def __init__(self, a, steps):
        self.a = a
        self.steps=steps
    def __len__(self):
    ...
    def is_finite(self):
        return True
    def __iter__(self):
        for i,c in zip(range(self.steps), self.a):
            yield c
    def clone(self):
        return Limit(self.a, self.steps)

class RainbowWave(AnimationMixin):
    def __init__(self, steps=30, intensity=255, offset=0.0):
        self.steps=steps
    ...
    def __iter__(self):
        ...
        while True:
            for i in range(self.steps):
                x = math.cos(math.pi*(i/self.steps+self.offset))
                y = math.cos(math.pi*(i/self.steps+1/3+self.offset))
                z = math.cos(math.pi*(i/self.steps+2/3+self.offset))
                yield (int(self.intensity*x*x), int(self.intensity*y*y), int(self.intensity*z*z))
    def clone(self):
        return RainbowWave(self.steps, self.intensity)
    def last(self):
        import math
        x = math.cos(math.pi*(self.offset))
        y = math.cos(math.pi*(1/3+self.offset))
        z = math.cos(math.pi*(2/3+self.offset))
        return (int(self.intensity*x*x), int(self.intensity*y*y), int(self.intensity*z*z))
...
ENCODER=(ENCODER_LEFT, ENCODER_RIGHT, ENCODER_SWITCH)
...
class Screen(App):
    ...
    def do_tick(self, container):
        ...
        if container.macropad.encoder_switch:
            for key in self.keys:
                if ENCODER_SWITCH == key.n:
                    key.do_action(self, container)
        if container.delta_encoder>0:
            for key in self.keys:
                if ENCODER_RIGHT == key.n:
                    for i in range(container.delta_encoder):
                        key.do_action(self, container)
        elif container.delta_encoder<0:
            for key in self.keys:
                if ENCODER_LEFT == key.n:
                    for i in range(-container.delta_encoder):
                        key.do_action(self, container)
